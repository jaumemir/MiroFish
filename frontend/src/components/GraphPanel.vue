...
const wasSimulating = ref(false) // tracks whether simulation was running previously
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not simulating; show the end hint
...
const renderGraph = () => {
  ...
  // Process edge data, calculate edge count and index between each pair of nodes
  const edgePairCount = {}
  ...
  // Track the index of the current edge being processed for each pair of nodes
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // stores detail info for all self-loop edges
        }
      })
      return
    }
    ...
    // Calculate curvature: spread multiple edges apart, keep single edges straight
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature values to ensure clear distinction
      // Curvature range increases with edge count — more edges means wider spread
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // If the edge direction is opposite to the canonical direction, flip the curvature
      // This ensures all edges are distributed in the same reference frame and do not overlap due to direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on edge count between the pair
    ...
    // Add gravity towards the centre so isolated node clusters are drawn to the centre
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset; calculated as a ratio of distance to keep curves clearly visible
  ...
  // Calculate the midpoint of a curve (for label positioning)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether dragging has truly started (moved beyond threshold)
      ...
      .on('end', (event, d) => {
        // Only let simulation wind down if a real drag occurred
        if (d._isDragging) {
...
/* Styles for the hint shown after simulation ends */
