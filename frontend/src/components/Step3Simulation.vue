...
const doStartSimulation = async () => {
  ...
  // Reset all state first to ensure we are not affected by the previous simulation
  resetAllState()
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: enableGraphMemoryUpdate.value  // enable dynamic graph memory update
    }
...
// Track last round per platform to detect changes and emit log entries
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether simulation is complete (via runner_status or platform completion state)
      ...
      // Determine via twitter_completed and reddit_completed flags
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform has completed, check whether all enabled platforms have
  // Determine whether a platform is enabled by its actions_count (> 0), running flag, or completed flag
...
onMounted(async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (status === 'failed') {
        // Previous run failed — start fresh (force=true will clean up old state)
      }
      if (status === 'stopped') {
    ...
    // If status check fails, fall through to start fresh
  }
