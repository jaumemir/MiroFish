    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to be applied (may have been overridden by later scroll)
          ...
          // Release lock after animation and check for pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation ends, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Wait briefly before applying to avoid too-rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
.grid-pattern {
  ...
  /* Positioned from top-left; height changes only extend at the bottom, not affecting existing grid position */
