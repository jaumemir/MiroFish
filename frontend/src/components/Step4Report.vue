            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but section may not be fully complete yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep the button at the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - matching "- Related facts: X" format
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interviewee count (e.g. "5 / 9 agents")
    ...
    // Extract agent selection rationale
    const reasonMatch = text.match(/### Agent selection rationale\n([\s\S]*?)(?=\n---\n|\n### Interview records)/)
    if (reasonMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (role description)
      ...
      // Extract answer - split into Twitter and Reddit
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform tags (very old format), use entire answer as-is
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (labeled format)
      // 2. "1. " or "\n1. " (numeric list, legacy format)
      let matches = []
      let match
    ...
    // Check if a question has answers from both platforms (excluding placeholder text)
...
const activeStep = computed(() => {
  ...
  return steps[0] || { noLabel: '--', title: 'Pending', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Strip leading level-2 heading (## xxx) since the section title is shown in the outer wrapper
  ...
  // Handle line breaks - blank lines become paragraph separators, single breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Remove <br> before paragraph tags following block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists: maintain incrementing count when <ol> items are separated by paragraph content
...
// Extract final answer content - parse section content from LLM response
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown formatting, try to strip the thinking process
